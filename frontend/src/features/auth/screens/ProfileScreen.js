...
export default function ProfileScreen({ navigation }) {
    ...
    const loadData = async () => {
        try {
            ...
            // Switch to profile endpoint which returns counts
            const res = await client.get(`/profile/${currentUid}`);
            setProfile(res.data);

            try {
                // We still fetch lists to populate if needed, or if backend profile doesn't have them
            ...
            >
                ...
                <View style={styles.profileHeader}>
                    ...
                    <View style={styles.statsRow}>
                        <TouchableOpacity style={styles.stat} onPress={() => navigation.navigate('ProfileDetail', { userId: uid })}>
                            <Text style={styles.statNum}>{stats.posts}</Text>
                            <Text style={styles.statLbl}>Posts</Text>
                        </TouchableOpacity>
                        <TouchableOpacity style={styles.stat} onPress={() => navigation.navigate('FollowList', { type: 'followers', userId: uid })}>
                            <Text style={styles.statNum}>{profile?.followers_count || stats.followers}</Text>
                            <Text style={styles.statLbl}>Followers</Text>
                        </TouchableOpacity>
                        <TouchableOpacity style={styles.stat} onPress={() => navigation.navigate('FollowList', { type: 'following', userId: uid })}>
                            <Text style={styles.statNum}>{profile?.following_count || stats.following}</Text>
