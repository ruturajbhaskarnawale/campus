...
def list_notifications(uid):
    try:
        ...
        notifs = session.query(Notification, User).outerjoin(User, Notification.sender_id == User.id)\
            .filter(Notification.recipient_id == user.id).order_by(Notification.created_at.desc()).all()
...
def mark_all_read():
    try:
        ...
        user = session.query(User).filter(User.uid == uid).first()
        if user:
            session.query(Notification).filter(Notification.recipient_id == user.id, Notification.is_read == False).update({Notification.is_read: True})
            session.commit()
